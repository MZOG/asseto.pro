...
function EquipmentCard({ props }: { props: EquipmentProps }) { // dodać typy
  return (
    <Link to="/dashboard/equipment/$id" params={{ id: props.id }} className="border border-gray-100 rounded-md p-3">
      <Skeleton className="size-40 w-full" />
      <h2 className="font-medium mt-3">{props.name}</h2>
      <p className='text-xs text-gray-500 mt-1'>Dodano: {formatDate(props.created_at, false)}</p>
      <p className='text-xs text-gray-500 mt-1'>Numer seryjny: {props.serial_number}</p>
